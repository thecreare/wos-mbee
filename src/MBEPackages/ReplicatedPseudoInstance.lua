...
local SortedArray = _Load_Library_("SortedArray")
local PseudoInstance = _Load_Library_("PseudoInstance")

local Templates = _Load_Library_("LocalTableTemplates")

local RemoteEvent = _Resource_Get_("RemoteEvent", "PseudoInstanceReplicator")
local RemoteFunction = _Resource_Get_("RemoteFunction", "PseudoInstanceStartupVerify")
...
local function YieldUntilReadyToFire(Player, ...)
	repeat until LoadedPlayers[Player] or not task.wait()
	FireClient(Player, ...)
end
...
elseif IsClient then
	...
	RemoteEvent.OnClientEvent:Connect(function(EventNumber, ClassName, Id, RawData, Assigned) -- Handle objects being replicated to clients
		repeat until OnClientEventNumber == EventNumber or not task.wait()

		local Template = Templates[ClassName]

		if not Template then
			_Load_Library_(ClassName)
