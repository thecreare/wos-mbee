local Compiler = {}
Compiler.DateAdded = "2022-07-23"
Compiler.Default = false -- selected by default

local Decompiler = require(script.Decompiler)

--local Malleability = require(script.Parent.Parent.Malleability)
local compress, decompress = unpack(require(script.Compression))
...
local function getPartData(part, getIssues)
...
function Compiler.Compile(Parts, Settings)
	...
	for i = 0, math.ceil(#Parts / 150) - 1 do
		...
		for i = 1 + PartOffset, 150 + PartOffset do
			coroutine.resume(coroutine.create(function()
				...
				local success, data, issue, err = pcall(getPartData, Parts[i], false)
				if success then
	...
	return compress(game:GetService("HttpService"):JSONEncode(result))
end
