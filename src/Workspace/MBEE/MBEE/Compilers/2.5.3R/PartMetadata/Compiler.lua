...
local function readPart(data: buffer, offset: number, config: SaveConfig)
	...
	if config.UseStateData then
		...
		local health = buffer.readu32(data, offset + PART_HEALTH_OFFSET)
...
local function writePart(data: buffer, offset: number, partData: PartDataSparse, config: SaveConfig)
	...
	if config.UseStateData then
		...
		buffer.writeu32(data, offset + PART_HEALTH_OFFSET, partData.Health or 0)
	end
end
...
local function decodeSaveData(saveData: string | buffer, config: SaveConfig): SaveData?
	...
	while offset < length do
		local _formatVersion = buffer.readu16(data, offset + VERSION_OFFSET)
		...
		local function loadPart(assembly: AssemblyData, offset: number)
			-- Load part fields
			local part = readPart(data, offset, config)
		...
		while offset < variableDataOffset do
			...
			-- Add the assembly
			table.insert(decoded.Assemblies, assembly)
		end
