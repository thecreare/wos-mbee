...
local Shadow = _Load_Library_("Shadow")
local RippleButton = _Load_Library_("RippleButton")
local Enumeration = _Load_Library_("Enumeration")
local PseudoInstance = _Load_Library_("PseudoInstance")
local RoStrapPriorityUI = _Load_Library_("RoStrapPriorityUI")
	...
	Methods = {
		Enter = function(self)
			...
			Tween(SnackbarFrame, "Position", self.EnterPosition, Deceleration, self.ENTER_TIME, false, function(Completed)
				if Completed == TweenCompleted and task.wait(self.DisplayTime) then
					while IsInputting(CurrentlyInputting) do
						repeat task.wait() until not IsInputting(CurrentlyInputting)
						task.wait(self.DisplayTime)
					end
