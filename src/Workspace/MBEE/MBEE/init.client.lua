...
function ModernDecompile(content)
	...
	local detailsScript = Instance.new("Script")

	detailsScript.Source = `-- The following is a summary of the data contained in the model's save format generated by LuaEncode. Your model has been selected in the explorer. \nreturn {LuaEncode(saveData, {
		Prettify = true
	})}`
	detailsScript.Name = "MBDetails"
	detailsScript.Archivable = false

	detailsScript.Parent = model

	plugin:OpenScript(detailsScript)

	warn('[MB:E:E] SUCCESSFULLY DECOMPILED DATA')
	...
	local success, err = pcall(function()
		...
		if #Compilation <= 200000 then
			...
			for i=0, math.ceil(#Compilation / 200000) - 1 do
				local OutputScript = Instance.new("Script")
				OutputScript.Name = "Output #" .. i + 1
				OutputScript.Source = string.sub(Compilation, 1 + 199999 * i, #Compilation >= (199999 + 199999 * i) and 199999 + 199999 * i or #Compilation)
				OutputScript.Parent = folder
				if OpenCompilerScripts then plugin:OpenScript(OutputScript) end
