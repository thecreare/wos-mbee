...
local ConfigData = require(script.Parent.Parent.ConfigData)
...
ModelBuilder.DateAdded = "2024-10-14"
ModelBuilder.Default = false -- selected by default
ModelBuilder.Selected = false

type ValueClassName = "StringValue" | "BoolValue" | "NumberValue" | "IntValue" | "DoubleConstrainedValue" | "IntConstrainedValue" | "CFrameValue" | "Vector3Value" | "Color3Value"
...
local function populateConfigurables(parent: Instance, configData: {PartMetadata.ConfigValue}, configValues: { [string | number]: unknown })
	...
	for _, configInfo in configData do
		...
		populateValue(parent, configName, if decodedValue == nil then decodeDefault(configInfo.Default) else decodedValue)
	end
end
